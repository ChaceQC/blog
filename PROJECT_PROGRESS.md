...
- M1 内容管理继续推进，文章封面选择与前台展示已形成第一版闭环；当前按用户要求继续推进列表密度、移动端适配和后台管理页展示结构优化，并继续参考 `https://innei.in/` 打磨公开展示与动效。
...
- 优先优化所有列表类界面，避免文章、文件、友链、站点目录、日志等数据过多时继续平铺堆满页面，改为分页、分组、搜索过滤、滚动容器或紧凑表格等更利于浏览的结构。
- 系统性修复移动端适配，重点检查公开页和后台管理页在窄屏下的导航、工具栏、列表、表单、封面图、按钮组和弹窗，避免文字溢出、布局重叠和横向撑开。
- 重构后台管理各页面的展示方式，避免新增功能直接向页面尾部堆积；优先采用顶部操作区、标签页、折叠分组、侧栏/抽屉编辑区和分段表单，让页面保持短而可扫描。
