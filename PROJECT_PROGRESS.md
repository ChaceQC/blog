...
- 修复公开评论列表一次只取前 100 条导致“显示更多”无法完整展示、回复被截断的问题。
- 修复父评论被标记为垃圾或拒绝后，仍公开的回复失去父楼层上下文的问题。
- 修复评论审核/删除后文章详情头部评论数缓存不随评论区一起刷新的问题。
- 修复本地最多保存 200 条 receipt 但 owned 接口最多接受 50 条导致找回失败的问题。
- 修复后台评论分页在最后一页条目被处理后页码越界的问题。
- 收紧匿名昵称冲突处理的并发边界，尽量保证不同匿名作者不会得到同一展示昵称。
...
- 公开评论分页需要兼容当前 `limit/offset` API，并避免一次性拉取过量评论。
- 昵称唯一性在缺少现成映射表的情况下需要用数据库约束或重试策略收紧，并保留同一匿名作者复用同一昵称的规则。

### 下一步

- 修改公开评论查询、前端分页加载、后台审核状态迁移和相关回归测试。

### 验证

- 待运行后端评论相关测试、前端 lint/build 与 receipt 单测。

## 2026-06-29

### 本轮计划

- 修复公开评论区展示密度：顶层评论默认展示 5 条，剩余通过“显示更多”继续展开。
- 修复二级回复展示密度：每个顶层楼层下的回复默认展示 3 条，剩余通过较小的“显示更多”继续展开。
- 修复长评论正文直接撑开页面的问题：默认折叠过长正文，点击“展开”后显示完整内容。
- 修复回复评论时表单位置不符合语义的问题：顶部表单只用于顶层评论，回复表单显示在被回复评论附近。
- 修复后台评论审核列表没有分页的问题，避免评论量增加后一次性展示过多。

### 已完成

- 公开评论区改为按顶层楼层展示：默认展示 5 条顶层评论，剩余通过“显示更多评论”继续展开。
- 每个顶层楼层下默认展示 3 条回复，剩余通过小号“显示更多回复”继续展开；本地持有 receipt 的评论会强制展示，避免提交者看不到自己的审核中评论。
- 顶部评论表单只用于顶层评论；点击任意已发布评论的“回复”后，在该评论附近显示内联回复表单。
- 过长评论正文默认折叠为预览文本，点击“展开”后显示完整纯文本内容。
- 后台评论审核列表接入分页，按 `limit=10` 和 `offset=page * 10` 请求，并复用 `ListPager`。
...
- 需要保证本轮只改变展示分页/折叠行为，不改变评论审核、删除凭证、XSS 纯文本渲染和二级回复数据语义。

### 下一步

- 提交并推送本轮前端评论展示优化；部署后用真实文章评论区检查移动端内联回复表单和后台评论分页。
...
- 新增 `docs/anonymous-comments-design.md`，明确评论系统不模拟公开账号，而是拆分为展示身份、风险身份和单条评论删除凭证。
- 设计匿名身份模型：本地 `comment.author_secret` 只用于派生匿名作者，公开展示使用按文章隔离的 `author_public_id`，删除使用每条评论独立 `delete_token`。
- 设计公开 API、后台审核 API、数据表、状态机、前端 receipt 管理、反垃圾限流、日志/遥测脱敏和测试清单。
- 补充评论安全威胁模型：明确 XSS/DOM XSS、CSRF/重放、IDOR、删除 token 爆破、刷评论/DoS、SQL 注入、日志注入、隐私关联和第三方资源 SSRF 的实现约束。

### 进行中

- 评论功能目前处于设计阶段，尚未实现数据库迁移、后端 API 或前端 UI。

### 阻塞与风险

- 无登录系统无法支持可靠跨设备找回删除权；用户清理浏览器数据或丢失删除凭证后，只能联系管理员删除。

### 下一步

- 若确认该方案，先实现 `post_comments` 迁移、评论 Service 和公开创建/删除 API，并把安全威胁模型中的 XSS、越权删除、限流和脱敏测试作为第一批回归测试。

### 验证

- 本轮为设计文档更新，未修改运行代码，未运行后端或前端测试。
- 已运行 `git diff --check`，未发现空白或行尾问题。
...
- 已将 `.post-detail-actions` 的桌面端位置改为根据正文宽度、按钮宽度和 24px 间距计算，优先放在文章正文右侧外缘。
- 已保留原有 `@media (max-width: 640px)` 移动端规则，手机端仍沿用底部悬浮布局。

### 验证

- 已运行 `npm.cmd run lint`，通过。
- 已运行 `npm.cmd run build`，通过；Vite/Rolldown 仍提示既有大 chunk 输出。
- 已运行 `git diff --check`，未发现空白或行尾问题。
...
- 公开访问日志压力已收敛：原先按类型跳过高频成功访问的策略已调整为 `BLOG_ACCESS_LOG_DEDUPE_SECONDS` 短时去重；同一 IP 在窗口内重复 `GET/HEAD` 同一 path 只写入第一条 `access_logs`，不把 query、临时 token 或签名参数纳入去重 key，生产 Redis 可用时使用 Redis `SET NX EX`，否则回落进程内缓存；错误访问、写操作和公开友链申请仍逐条记录，后台短时链接和后台下载等成功 `GET/HEAD` 也遵循同一短时去重策略。
...
- P2 临时访问 token 和加密 session header 长度上限已补齐：公开下载 token、文章图片 token、后台预览 token 统一限制到 2048 字符以内，`X-Encryption-Session` 头与加密信封 `session_id` 一样限制到 128 字符以内，超长值会在进入 HMAC/base64/数据库比较前被拒绝。该修复不涉及数据库字段、索引或服务器环境变量变化，不需要新增 Alembic 迁移。
...
- P4 访问日志短时去重文档口径已同步：确认当前实现按用户此前确定的 `IP + method + path` 短窗口策略处理所有成功 `GET/HEAD`，包括后台短时链接和后台文件下载；错误、写操作和公开友链申请仍逐条记录。已同步 `backend/README.md` 与本进度文件，不改运行逻辑、不涉及数据库迁移或服务器配置。
- P4 大体量测试文件已按领域拆分：`test_public_content_api.py` 拆出公开加密、feed/SEO、公开链接和公开设置测试，原文件从 1421 行降到 377 行；`test_admin_links_api.py` 拆出分组和站点导航测试，原文件从 893 行降到 151 行；`test_admin_files_api.py` 拆出公开文件、后台文件访问和文章图片测试，原文件从 865 行降到 165 行；共享 fake/override 收敛到 helper 模块。该调整不改业务逻辑、不涉及数据库迁移或服务器配置。
- 本轮待修复项收尾全量验证已完成：后端 lint/pytest、Alembic SQL、前端 lint/test/build 和 Compose 配置展开均通过；当前待修复清单已清空。
- 已按项目分支规则将 `dev` 快进合并到 `main` 并推送，`dev`、`main`、`origin/dev` 和 `origin/main` 当前均指向同一收尾提交。服务器侧可按发布流程拉取 `main` 后重新构建后端与前端静态产物。
...
- 本轮待修复清单已处理完成，全量验证已通过，`dev` 已合并并推送到 `main`。
...
- 下一步在已部署服务器按发布流程拉取 `main`，重新构建后端和前端静态产物，执行 Alembic 到 head，并复核 `/api/health`、后台登录、公开文章资源缓存、日志 IP 和访问日志短时去重效果。

### 验证

- 本轮追加只读审计已运行 `rg` 扫描限流 key、认证时序、临时 token/header 长度、SSRF 解析与跳转、Markdown sanitize、前端危险 sink、AbortSignal、依赖配置、部署端口和源码/测试体量；新增问题已写入“待修复清单”。
...
- P2 token/header 长度上限修复后已运行 `uv run ruff check app/api/encrypted_response.py app/api/public/files.py app/api/admin/files_common.py app/schemas/files.py tests/test_admin_files_api.py tests/test_admin_encryption_api.py`，通过。
- P2 token/header 长度上限修复后已运行 `uv run pytest tests/test_admin_files_api.py tests/test_admin_encryption_api.py`，30 个测试通过；仍存在 5 个 FastAPI/Starlette 上游弃用警告。
- P3 友链健康检查 DNS rebinding 修复后已运行 `uv run ruff check app/tasks/links.py tests/test_link_health.py`，通过。
- P3 友链健康检查 DNS rebinding 修复后已运行 `uv run pytest tests/test_link_health.py`，5 个测试通过。
- P3 前端加密会话协商取消链路修复后已运行 `npm.cmd run lint -- src/api/encryption.ts src/api/encryption.test.ts`，通过。
- P3 前端加密会话协商取消链路修复后已运行 `npm.cmd test`，4 个测试文件 7 个测试通过。
- P3 前端加密会话协商取消链路修复后已运行 `npm.cmd run build`，通过；Vite 仍提示单个主 chunk 超过 500 kB 的既有体积告警。
- P3 Markdown 正文外链图片策略收紧后已运行 `uv run ruff check app/providers/markdown.py tests/test_markdown_provider.py`，通过。
...
- P4 访问日志短时去重文档口径同步后已运行 `rg "后台短时链接|后台下载|逐条记录|短时去重|BLOG_ACCESS_LOG_DEDUPE_SECONDS" README.md backend/README.md PROJECT_PLAN.md PROJECT_PROGRESS.md`，确认文档统一描述为成功 `GET/HEAD` 按 IP、方法和 path 短时去重，错误和写操作逐条记录。
- P4 大体量测试文件拆分后已运行 `uv run ruff check tests/test_public_content_api.py tests/test_public_encryption_api.py tests/test_public_feeds_api.py tests/test_public_links_api.py tests/test_public_settings_api.py tests/public_content_api_helpers.py tests/test_admin_links_api.py tests/test_admin_link_groups_api.py tests/test_admin_site_nav_api.py tests/admin_links_api_helpers.py tests/test_admin_files_api.py tests/test_public_files_api.py tests/test_admin_file_access_api.py tests/test_post_file_images_api.py tests/admin_files_api_helpers.py`，通过。
- P4 大体量测试文件拆分后已运行 `uv run pytest tests/test_public_content_api.py tests/test_public_encryption_api.py tests/test_public_feeds_api.py tests/test_public_links_api.py tests/test_public_settings_api.py tests/test_admin_links_api.py tests/test_admin_link_groups_api.py tests/test_admin_site_nav_api.py tests/test_admin_files_api.py tests/test_public_files_api.py tests/test_admin_file_access_api.py tests/test_post_file_images_api.py`，66 个测试通过；仍存在 FastAPI/Starlette TestClient、HTTP 状态常量和 per-request cookies 的上游弃用警告。
- P4 大体量测试文件拆分后已运行测试文件体量扫描，三个待修复目标文件分别降为 `test_public_content_api.py` 377 行、`test_admin_links_api.py` 151 行、`test_admin_files_api.py` 165 行。
- 本轮待修复项收尾后已运行 `uv run ruff check .`，通过。
- 本轮待修复项收尾后已运行 `uv run pytest`，226 个测试通过、2 个 Redis 集成测试因未设置 `BLOG_TEST_REDIS_URL` 跳过；仍存在 7 个 FastAPI/Starlette 上游弃用警告。
- 本轮待修复项收尾后已运行 `uv run alembic upgrade head --sql`，可正常生成从空库到 `20260619_0009` 的 MySQL 迁移 SQL。
- 本轮待修复项收尾后已运行 `npm.cmd run lint`，通过。
- 本轮待修复项收尾后已运行 `npm.cmd test`，4 个测试文件 7 个测试通过。
...
- 本轮待修复项收尾后已执行 `git merge --ff-only dev` 并推送 `main`；随后切回 `dev` 并确认 `dev`、`main`、`origin/dev`、`origin/main` 均包含收尾提交。
...
- 本轮继续全量审计已确认 `backend/migrations/versions/__pycache__` 只是本地 ignored 产物，未被 Git 追踪。
- 本轮继续全量审计已运行 `uv run ruff check .`，通过。
- 本轮继续全量审计已运行 `uv run pytest`，210 个测试通过，2 个 Redis 集成测试因未设置 `BLOG_TEST_REDIS_URL` 跳过；仍存在 7 个 FastAPI/Starlette 上游弃用警告。
...
- RSS/sitemap 路由拆分后已运行 `uv run ruff check app/api/public/feeds.py app/api/public/feed_cache.py app/api/public/feed_renderers.py`，通过。
...
- 已优化文章列表移动端布局：小屏不再使用序号、封面、正文三列挤压布局，改为序号左侧固定、封面和正文在右侧内容列纵向展开，底部日期保持左下，阅读时长、字数、浏览数和点赞数保持右下横向扫读。
...
- 移动端文章列表重排后已重新运行 `npm.cmd run lint`，通过。
- 移动端文章列表重排后已重新运行 `npm.cmd run build`，通过；Vite/Rolldown 仍提示混淆插件耗时较高。
- 移动端文章列表重排后已使用 Playwright + Microsoft Edge 实测 `/posts`：390px 视口无横向溢出，封面在正文上方，标题/摘要有 298px 内容宽度，日期左下、统计右下且统计保持单行；360px 视口同样无横向溢出，正文宽度 280px，统计保持单行。
- 已运行 `git diff --check`，未发现空白或行尾问题。
