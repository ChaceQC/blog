...
- 登录、加密协商、刷新令牌、上传、友链申请、搜索等接口做限流。当前 M1 第一版已对后台登录和加密协商入口接入单进程内存限流，并在命中时写入 `security_events`；生产多实例或多进程部署前需要替换为 Redis 等共享存储适配器。
...
- 当前最小实现使用 `/api/admin/encryption/sessions` 和 `/api/public/encryption/sessions` 协商浏览器 P-256 ECDH 短期会话密钥，协商结果保存到 MySQL `encryption_sessions` 表，并通过不同 scope 区分后台与公开接口颁发范围；`/api/admin/auth/login`、`/api/admin/auth/refresh` 和 `/api/admin/auth/me` 必须在请求携带 `X-Encryption-Session` 时返回 `sensitive-v1` 加密信封，不再保留旧的明文 JSON 响应形态。后台登录与加密协商入口已接入可配置限流，命中写入 `security_events`。后台文章与页面管理接口创建、更新、预览请求和响应已接入 `content-v1`，公开文章、友链、站点目录、站点资料和公开文件列表也已接入 public scope 的 `content-v1` 加密响应。过期加密会话已提供 CLI 清理入口，后续需继续补充 Redis 限流适配、更多审计记录和定时调度配置。
...
5. 评估并实现 Redis 共享限流适配器，替换或扩展当前单进程内存限流器，优先覆盖后台登录和加密协商入口，并保留无 Redis 时的本地降级策略。
6. 后台维护任务体系已开始落地，当前已提供过期加密会话清理、软删除文件物理清理和孤儿文件 dry-run/显式清理 CLI 入口，并已通过真实 MySQL 临时库验证文件清理闭环；下一步补充定时调度配置建议。
