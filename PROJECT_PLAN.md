...
- 私有文件必须通过后台鉴权下载，不能由 Nginx 直接暴露目录。
...
5. 将文件管理继续接入私有文件鉴权下载、更多使用场景引用追踪和物理清理任务。
6. 后台维护任务体系已开始落地，当前已提供过期加密会话 CLI 清理入口；下一步将文件管理接入私有文件鉴权下载、孤儿文件/软删除文件物理清理任务，并将当前单进程内存限流器替换或扩展为 Redis 共享存储适配器。
