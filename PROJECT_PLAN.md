...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页与 Repository、Service、Policy 边界，下一步继续补齐后台权限闭环和加密传输基础：

1. 补充认证相关审计日志、登录日志查询和基础限流策略。
2. 接入 `sensitive-v1` 和 `content-v1` 应用层加密信封的密钥协商、接口约定和前端解密流程。
