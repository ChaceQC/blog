...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。下一阶段进入 M1，先做认证与后台框架：

1. 实现后台登录、刷新令牌和退出接口。
2. 建立用户、角色、权限的 Repository、Service 和 Policy 边界。
3. 接入后台权限路由、菜单状态和登录态保护。
4. 补充认证相关审计日志、登录日志和基础限流策略。
5. 为认证服务补充后端测试，并同步前端后台登录页。
