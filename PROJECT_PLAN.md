...
- 登录、加密协商、刷新令牌、上传、友链申请、搜索等接口做限流。当前 M1 已对后台登录和加密协商入口接入可配置限流，并在命中时写入 `security_events`；本地默认使用单进程内存限流，生产可通过 `BLOG_RATE_LIMIT_BACKEND=redis` 和 `BLOG_REDIS_URL` 切换到 Redis 共享限流，Redis 不可用时回落到进程内限流器。
...
- `content-v1`：用于文章、页面、草稿、私有内容和内容管理接口。草稿、私有文章和后台传输使用内容密钥加密；公开文章、友链、站点目录、站点资料和公开文件列表通过 public scope 会话返回 `content-v1` 加密信封。当前后台文章与页面管理接口的创建、更新、预览请求和响应已使用 `content-v1` 加密信封。
- 两套加密信封必须使用不同用途标识、不同派生上下文和独立密钥轮换策略；日志中禁止输出明文、密钥、nonce、完整密文和解密失败详情。
- 应用层加密不能替代 HTTPS、HttpOnly Cookie、CSRF、权限校验和 HTML sanitize。
- 当前最小实现使用 `/api/admin/encryption/sessions` 和 `/api/public/encryption/sessions` 协商浏览器 P-256 ECDH 短期会话密钥，协商结果保存到 MySQL `encryption_sessions` 表，并通过不同 scope 区分后台与公开接口颁发范围；`/api/admin/auth/login`、`/api/admin/auth/refresh` 和 `/api/admin/auth/me` 必须在请求携带 `X-Encryption-Session` 时返回 `sensitive-v1` 加密信封，不再保留旧的明文 JSON 响应形态。后台登录与加密协商入口已接入可配置限流，支持内存与 Redis 共享后端，命中写入 `security_events`。后台文章与页面管理接口创建、更新、预览请求和响应已接入 `content-v1`，公开文章、友链、站点目录、站点资料和公开文件列表也已接入 public scope 的 `content-v1` 加密响应。过期加密会话已提供 CLI 清理入口，后续需继续补充更多审计记录和定时调度配置。
...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、后台日志查询、登录与加密协商入口限流、Redis 共享限流适配器、后台文章与页面管理接口、后台文章实时预览、后台文件列表上传软删除和签名预览、后台站点基础设置保存接口、后台友链创建编辑审核、导航条目读取创建编辑接口、公开文章列表与详情读取链路、公开站点资料读取、公开文件栏、文章封面和正文图片引用追踪、文章封面后台选择器、无封面默认封面、首页碎念与社交入口后台维护、首页近期笔墨 3 篇限制、文章列表分页、前台文章列表与详情封面展示、公开文件短时访问、后台私有文件鉴权下载、软删除文件物理清理任务、孤儿文件 dry-run 扫描与显式清理任务、真实 MySQL 临时库文件清理闭环验证、前台与后台 KaTeX 公式渲染、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐内容管理基础：

1. 优先优化所有列表类界面，避免文章、文件、友链、站点目录、日志等数据过多时继续平铺堆满页面，改为分页、分组、搜索过滤、滚动容器或紧凑表格等更利于浏览的结构。
2. 系统性修复移动端适配，重点检查公开页和后台管理页在窄屏下的导航、工具栏、列表、表单、封面图、按钮组和弹窗，避免文字溢出、布局重叠和横向撑开。
3. 重构后台管理各页面的展示方式，避免新增功能直接向页面尾部堆积；优先采用顶部操作区、标签页、折叠分组、侧栏/抽屉编辑区和分段表单，让页面保持短而可扫描。
4. 前台友链、站点目录、文件页和文章详情继续参考 `https://innei.in/` 的轻时间线、双栏信息区、横向写作轨迹、顶部悬浮导航和克制动效做细节打磨。
5. 后台维护任务体系已开始落地，当前已提供过期加密会话清理、软删除文件物理清理和孤儿文件 dry-run/显式清理 CLI 入口，并已通过真实 MySQL 临时库验证文件清理闭环；生产部署已补充 systemd timer 示例，默认每小时清理过期加密会话、每天清理软删除文件、每周 dry-run 扫描孤儿文件。
6. 使用真实 Redis 或 Docker Compose 私有 Redis 验证后台登录和加密协商入口限流在共享后端下的 429、`Retry-After` 与安全事件记录。
7. 使用真实 MySQL 运行库继续验证后台上传图片、选择文章封面、复制文章引用、文章实时预览、发布文章、前台列表与详情封面展示、前台详情图片渲染、公开文件栏下载和后台访问日志查询。
