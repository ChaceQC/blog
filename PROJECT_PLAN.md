...
1. M5 安全收口：复查 HTML sanitize、安全响应头、CORS、Trusted Host、Cookie、CSRF、限流和敏感日志，确保公网部署默认不暴露内部端口与私有上传目录。
2. M5 部署验收：复查 Docker Compose、Nginx、环境变量示例、systemd 维护任务和生产端口暴露策略，确保 Debian 部署入口与当前代码一致。
3. M5 运维验收：复查 MySQL 与上传文件备份恢复脚本、维护任务执行路径和灾备说明，形成上线前最小演练清单。
