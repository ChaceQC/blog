...
- `content-v1`：用于文章、页面、草稿、私有内容和内容管理接口。草稿、私有文章和后台传输使用内容密钥加密；公开已发布文章仍以 HTTPS 传输明文渲染结果，避免破坏 SEO、RSS、缓存和公开访问体验。当前后台文章与页面管理接口的响应已使用 `content-v1` 加密信封，创建和更新请求仍是 HTTPS 下的普通 JSON，后续需要补充请求解密。
- 两套加密信封必须使用不同用途标识、不同派生上下文和独立密钥轮换策略；日志中禁止输出明文、密钥、nonce、完整密文和解密失败详情。
- 应用层加密不能替代 HTTPS、HttpOnly Cookie、CSRF、权限校验和 HTML sanitize。
- 当前最小实现使用 `/api/admin/encryption/sessions` 协商浏览器 P-256 ECDH 短期会话密钥，协商结果保存到 MySQL `encryption_sessions` 表；`/api/admin/auth/login`、`/api/admin/auth/refresh` 和 `/api/admin/auth/me` 必须在请求携带 `X-Encryption-Session` 时返回 `sensitive-v1` 加密信封，不再保留旧的明文 JSON 响应形态。后台登录与加密协商入口已接入可配置限流，命中写入 `security_events`。后台文章与页面管理接口响应已接入 `content-v1`。后续需继续补充请求解密、会话清理任务、Redis 限流适配和更多审计记录。

### 9.3 内容安全

- Markdown 与 LaTeX 渲染结果必须 HTML sanitize，公式渲染输出不得绕过清洗策略。
- 当前后台内容管理第一版使用临时安全渲染器生成 `content_html`，只做 HTML 转义和段落包裹；后续必须替换为正式 Markdown/LaTeX 渲染与 sanitize 策略。
...
1. 补充 `content-v1` 请求解密，完善后台文章和页面前端管理页面。
2. 替换临时安全渲染器，接入正式 Markdown/LaTeX 渲染与 HTML sanitize 策略。
