...
1. 下一步优先收敛前台友链、站点目录和文件页的展示密度与移动端验证，避免数据较多时页面过长、分页入口不明显或产生横向溢出。
...
6. Redis 共享限流已通过真实 Redis 集成测试验证；真实运行库 HTTP 级闭环验证脚本已覆盖后台上传图片、选择封面、文章实时预览、发布文章、定时发布不提前公开、分类、标签、SEO 信息、分类/标签稳定 URL、sitemap 分类/标签 URL、前台列表与详情封面展示、前台详情图片渲染、公开文件栏下载和后台访问日志查询。下一步继续把前台公开页的数据密度和移动端体验纳入截图或 HTTP 级检查。
