...
- 记录结构化访问日志和应用日志；后台短时链接生成、后台文件下载、公开友链申请、公开站点跳转、RSS/sitemap/robots 和错误访问写入 `access_logs`，高频公开内容读取、公开文件下载、文章图片渲染和缩略图成功访问默认可通过 `BLOG_ACCESS_LOG_SKIP_TYPES` 跳过写库，减少公开流量写放大。
