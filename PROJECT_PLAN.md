...
1. 前台友链、站点目录、文件页和文章详情继续参考 `https://innei.in/` 的轻时间线、双栏信息区、横向写作轨迹、顶部悬浮导航和克制动效做细节打磨。
2. 将文件管理继续接入私有文件鉴权下载、更多使用场景引用追踪和物理清理任务。
3. 补充加密会话过期清理任务，并将当前单进程内存限流器替换或扩展为 Redis 共享存储适配器。
4. 使用真实 MySQL 运行库继续验证后台上传图片、选择文章封面、复制文章引用、文章实时预览、发布文章、前台列表与详情封面展示、前台详情图片渲染、公开文件栏下载和后台访问日志查询。
