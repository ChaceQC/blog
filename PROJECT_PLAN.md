...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、后台日志查询、登录与加密协商入口限流、后台文章与页面管理接口、后台文章实时预览、后台文件列表上传软删除和签名预览、后台站点基础设置保存接口、后台友链创建编辑审核、导航条目读取创建编辑接口、公开文章列表与详情读取链路、公开站点资料读取、公开文件短时访问、前台与后台 KaTeX 公式渲染、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐内容管理基础：

1. 补齐公开文件栏，只展示专门公开展示的文件，并让下载按钮按当前 session 生成短时签名链接。
2. 将文件管理继续接入文章封面、正文图片引用追踪和私有文件鉴权下载。
3. 补充加密会话过期清理任务，并将当前单进程内存限流器替换或扩展为 Redis 共享存储适配器。
4. 使用真实 MySQL 运行库继续验证后台上传图片、复制文章引用、文章实时预览、发布文章、前台详情图片渲染和后台访问日志查询。
