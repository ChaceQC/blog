...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、后台日志查询、登录与加密协商入口限流、后台文章与页面管理接口、后台文章与页面前端管理页、后台文件/友链/导航/设置界面入口、后台站点基础设置保存接口、后台友链创建编辑审核、导航条目读取创建编辑接口、公开文章列表与详情读取链路、前台与后台 KaTeX 公式渲染、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐内容管理基础：

1. 继续补齐后台文件上传的真实 Admin API，形成最小文件管理闭环。
