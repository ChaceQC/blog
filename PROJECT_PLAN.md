...
- 当前最小实现使用 `/api/admin/encryption/sessions` 协商浏览器 P-256 ECDH 短期会话密钥，协商结果保存到 MySQL `encryption_sessions` 表；`/api/admin/auth/login`、`/api/admin/auth/refresh` 和 `/api/admin/auth/me` 必须在请求携带 `X-Encryption-Session` 时返回 `sensitive-v1` 加密信封，不再保留旧的明文 JSON 响应形态。后台登录与加密协商入口已接入可配置限流，命中写入 `security_events`。后台文章与页面管理接口创建、更新请求和响应已接入 `content-v1`。后续需继续补充会话清理任务、Redis 限流适配和更多审计记录。
...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、后台日志查询、登录与加密协商入口限流、后台文章与页面管理接口、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐内容管理基础：

1. 完善后台文章和页面前端管理页面，调用 `content-v1` 加密请求与响应。
2. 为前台和后台预览接入 KaTeX 等公式样式，确保 `content_html` 中的公式节点可正确展示。
3. 继续实现文件和后台设置的最小 CRUD 闭环。
4. 补充加密会话过期清理任务，并将当前单进程内存限流器替换或扩展为 Redis 共享存储适配器。
