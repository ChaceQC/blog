...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口与 Repository、Service、Policy 边界，下一步继续补齐后台登录闭环：

1. 补充初始管理员创建方式，避免真实数据库接入后无法登录后台。
2. 接入后台权限路由、菜单状态和登录态保护。
3. 同步前端后台登录页和 API 客户端认证状态管理。
4. 补充认证相关审计日志、登录日志查询和基础限流策略。
5. 在真实 MySQL 环境验证登录、刷新令牌、退出和迁移回滚。
