...
- 当前最小实现使用 `/api/admin/encryption/sessions` 协商浏览器 P-256 ECDH 短期会话密钥，`/api/admin/auth/login`、`/api/admin/auth/refresh` 和 `/api/admin/auth/me` 在请求携带 `X-Encryption-Session` 时返回 `sensitive-v1` 加密信封。后端暂以进程内存保存协商会话，仅用于单进程开发和最小闭环验证；多实例生产部署前必须改为 Redis 或数据库存储，并补充会话清理、审计和限流。
