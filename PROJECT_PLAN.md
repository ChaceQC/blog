...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、初始管理员创建方式、前端登录页与 Repository、Service、Policy 边界，下一步继续补齐后台权限闭环：

1. 补充后台当前用户接口和 Bearer Token 鉴权依赖。
2. 接入权限依赖、菜单权限状态和更细的后台路由保护。
3. 补充认证相关审计日志、登录日志查询和基础限流策略。
4. 继续实现文章、文件和后台设置的最小 CRUD 闭环。
5. 在真实 MySQL 环境持续验证后续认证、权限和内容接口。
