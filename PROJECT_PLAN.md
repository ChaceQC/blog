...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、后台日志查询、登录与加密协商入口限流、后台文章与页面管理接口、后台文章与页面前端管理页、公开文章列表与详情读取链路、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐内容管理基础：

1. 为前台和后台预览接入 KaTeX 等公式样式，确保 `content_html` 中的公式节点可正确展示。
2. 继续实现文件和后台设置的最小 CRUD 闭环。
3. 补充加密会话过期清理任务，并将当前单进程内存限流器替换或扩展为 Redis 共享存储适配器。
4. 将公开文章链路纳入真实 MySQL 临时库联调，继续验证后台发布后前台首页、文章列表和文章详情可见。
