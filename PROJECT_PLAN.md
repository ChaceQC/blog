...
- 遥测上报使用 Project API Key，不复用后台登录 token，不由客户端传 `project_id`。第一阶段只允许服务端、后台维护任务和部署脚本持有遥测 API Key；浏览器前端如需 Web Vitals 或 JS 错误上报，必须通过后端受控转发。遥测数据契约见 `docs/telemetry-reporting-design.md`，覆盖 HTTP 耗时/错误、限流、加密会话、salt WSS、公开文章互动、文件上传/访问、后台审计、友链申请、站点跳转和维护任务；所有 tags 必须使用路由模板和低基数字段，不上报正文、slug、完整 URL/query、签名 token、Cookie、加密材料、原始 IP、完整 UA、文件名、MIME、外部 URL 或完整设置值。
