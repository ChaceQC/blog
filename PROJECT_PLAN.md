...
- 当前最小实现使用 `/api/admin/encryption/sessions` 协商浏览器 P-256 ECDH 短期会话密钥，协商结果保存到 MySQL `encryption_sessions` 表；`/api/admin/auth/login`、`/api/admin/auth/refresh` 和 `/api/admin/auth/me` 必须在请求携带 `X-Encryption-Session` 时返回 `sensitive-v1` 加密信封，不再保留旧的明文 JSON 响应形态。后续需继续补充会话清理任务、审计记录和接口限流。
...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐后台权限闭环、安全审计和内容管理基础：

1. 补充认证相关审计日志查询、加密协商接口限流、登录接口限流和限流命中日志。
2. 将 `content-v1` 接入文章、页面和草稿管理接口，明确公开内容与后台内容的加密边界。
3. 继续实现文章、文件和后台设置的最小 CRUD 闭环。
