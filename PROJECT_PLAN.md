...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、后台日志查询、登录与加密协商入口限流、Redis 共享限流适配器、后台文章与页面管理接口、后台文章实时预览、后台文件列表上传软删除和签名预览、后台站点基础设置保存接口、后台友链创建编辑审核、导航条目读取创建编辑接口、公开友链申请入口、友链状态检查任务、公开文章列表与详情读取链路、公开站点资料读取、公开文件栏、RSS、sitemap、robots.txt 初版和生产 Nginx 精确反代规则、文章封面和正文图片引用追踪、文章封面后台选择器、无封面默认封面、文章定时发布、分类、标签、SEO 关键词、首页碎念与社交入口后台维护、首页近期笔墨 3 篇限制、文章列表分页、前台文章列表与详情封面展示、公开文件短时访问、后台私有文件鉴权下载、软删除文件物理清理任务、孤儿文件 dry-run 扫描与显式清理任务、真实 MySQL 临时库文件清理闭环验证、真实运行库 HTTP 级文章发布闭环验证脚本、前台与后台 KaTeX 公式渲染、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐内容管理基础：

1. 下一步优先推进公开页面 canonical 与 Open Graph 元信息基线：文章详情、文章列表、友链和站点目录需要补齐可分享标题、描述、canonical URL 和基础 `og:*`。
...
6. Redis 共享限流已通过真实 Redis 集成测试验证；真实运行库 HTTP 级闭环验证脚本已覆盖后台上传图片、选择封面、文章实时预览、发布文章、定时发布不提前公开、分类、标签、SEO 信息、前台列表与详情封面展示、前台详情图片渲染、公开文件栏下载和后台访问日志查询。下一步继续推进公开页面 canonical 与 Open Graph 元信息基线。
