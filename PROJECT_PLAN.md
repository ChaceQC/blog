...
版本：v0.1.0  
日期：2026-06-15  
技术约束：后端 Python + FastAPI + uv，前端 React + npm，数据库 MySQL，支持文件管理、友链、小网站跳转、后台管理。开发环境为 Windows 11，生产部署环境为 Linux Debian。
...
- 建立统一代码风格和环境变量模板。
- 创建并持续维护 `PROJECT_PROGRESS.md`，每次实现、重构、测试和部署调整都更新进度。
...
1. 创建 `backend` 与 `frontend` 双目录。
2. 用 `uv` 初始化 FastAPI 工程。
3. 用 `npm` 初始化 React + Vite 工程。
4. 创建生产化 Docker Compose，区分 public/private 网络。
5. 配置 Nginx、HTTPS 证书、只开放 `80/443` 的公网入口。
6. 创建 MySQL 内网服务、备份目录和环境变量模板。
7. 先落地用户、文章、文件、友链、导航站点核心表。
8. 再实现后台登录、文章 CRUD、文件上传和健康检查。
...
实现阶段必须维护 `PROJECT_PROGRESS.md`，并在以下时机实时更新：
