...
- 登录、加密协商、刷新令牌、上传、友链申请、搜索等接口做限流。当前 M1 已对后台登录、加密协商入口和公开友链申请入口接入可配置限流，并在命中时写入 `security_events`；加密 session 协商默认提高到 `120/分钟`，公开/后台活跃 session 默认上限分别为 `60` 和 `30`，避免冷缓存刷新或多公开查询并发时只完成 session 协商就被挡住。本地默认使用单进程内存限流，生产可通过 `BLOG_RATE_LIMIT_BACKEND=redis` 和 `BLOG_REDIS_URL` 切换到 Redis 共享限流，Redis 不可用时回落到进程内限流器。应用只在直接连接来源属于 `BLOG_TRUSTED_PROXY_HOSTS` 配置的 IP 或 CIDR 时信任 `X-Forwarded-For` / `X-Real-IP`，避免后端直连时伪造客户端 IP。
