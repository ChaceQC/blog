...
- `content-v1`：用于文章、页面、草稿、私有内容和内容管理接口。草稿、私有文章和后台传输使用内容密钥加密；公开已发布文章仍以 HTTPS 传输明文渲染结果，避免破坏 SEO、RSS、缓存和公开访问体验。当前后台文章与页面管理接口的创建、更新请求和响应已使用 `content-v1` 加密信封。
- 两套加密信封必须使用不同用途标识、不同派生上下文和独立密钥轮换策略；日志中禁止输出明文、密钥、nonce、完整密文和解密失败详情。
- 应用层加密不能替代 HTTPS、HttpOnly Cookie、CSRF、权限校验和 HTML sanitize。
- 当前最小实现使用 `/api/admin/encryption/sessions` 协商浏览器 P-256 ECDH 短期会话密钥，协商结果保存到 MySQL `encryption_sessions` 表；`/api/admin/auth/login`、`/api/admin/auth/refresh` 和 `/api/admin/auth/me` 必须在请求携带 `X-Encryption-Session` 时返回 `sensitive-v1` 加密信封，不再保留旧的明文 JSON 响应形态。后台登录与加密协商入口已接入可配置限流，命中写入 `security_events`。后台文章与页面管理接口创建、更新请求和响应已接入 `content-v1`，前端后台文章与页面管理页已复用同一套加密请求和响应解密流程。后续需继续补充会话清理任务、Redis 限流适配和更多审计记录。
...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、后台日志查询、登录与加密协商入口限流、后台文章与页面管理接口、后台文章与页面前端管理页、后台文件/友链/导航/设置界面入口、后台站点基础设置保存接口、后台友链审核与导航条目读取接口、公开文章列表与详情读取链路、前台与后台 KaTeX 公式渲染、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐内容管理基础：

1. 继续补齐后台文件上传、友链创建编辑和导航写入的真实 Admin API，形成最小 CRUD 闭环。
2. 补充加密会话过期清理任务，并将当前单进程内存限流器替换或扩展为 Redis 共享存储适配器。
3. 将公开文章链路和 KaTeX 渲染纳入真实 MySQL 临时库联调，继续验证后台发布后前台首页、文章列表和文章详情可见。
