...
M0 已完成仓库结构、后端数据模型、初始迁移、前端骨架和部署骨架。M1 已开始落地后端认证接口、当前用户接口、Cookie 会话、CSRF、权限菜单、初始管理员创建方式、前端登录页、后台日志查询、登录与加密协商入口限流、Redis 共享限流适配器、后台文章与页面管理接口、后台文章实时预览、后台文件列表上传软删除和签名预览、后台站点基础设置保存接口、后台友链创建编辑审核、导航条目读取创建编辑接口、公开文章列表与详情读取链路、公开站点资料读取、公开文件栏、文章封面和正文图片引用追踪、文章封面后台选择器、无封面默认封面、首页碎念与社交入口后台维护、首页近期笔墨 3 篇限制、文章列表分页、前台文章列表与详情封面展示、公开文件短时访问、后台私有文件鉴权下载、软删除文件物理清理任务、孤儿文件 dry-run 扫描与显式清理任务、真实 MySQL 临时库文件清理闭环验证、真实运行库 HTTP 级文章发布闭环验证脚本、前台与后台 KaTeX 公式渲染、`sensitive-v1` / `content-v1` 应用层加密协商基础与 Repository、Service、Policy 边界，下一步继续补齐内容管理基础：

1. 优先用真实前端页面跑通核心写作发布闭环：登录后台、上传图片、在文章编辑页选择封面、插入正文图片、实时预览 Markdown/LaTeX、发布文章，并在前台首页、文章列表和文章详情确认封面与正文图片可见。
2. 围绕核心闭环修复真实使用缺口：文章编辑保存/发布错误提示、封面选择器、文件引用复制、预览图签名、公开图片渲染、发布后返回或跳转、前台空态与失败态。
3. 补齐轻量 CMS 的核心内容管理动作：文章编辑、归档/取消发布、页面管理、文件替换或引用查看、友链与导航基础维护，确保不是只能“创建一次”，而是能长期维护。
4. 在核心闭环稳定后，再继续优化列表密度、移动端适配、后台页面分区和前台友链/站点目录/文件页的展示细节。
...
6. Redis 共享限流已通过真实 Redis 集成测试验证；真实运行库 HTTP 级闭环验证脚本已覆盖后台上传图片、选择封面、文章实时预览、发布文章、前台列表与详情封面展示、前台详情图片渲染、公开文件栏下载和后台访问日志查询。下一步紧贴核心目标，用浏览器从真实前端跑通并修复“登录后台，上传图片，写文章，前台看到文章”的人工可用闭环。
