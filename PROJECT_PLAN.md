...
5. 使用真实 MySQL 临时库和临时上传目录验证上传、软删除、软删除文件清理、孤儿文件 dry-run 与显式删除，确认数据库记录与本地存储之间的清理闭环。
6. 后台维护任务体系已开始落地，当前已提供过期加密会话清理、软删除文件物理清理和孤儿文件 dry-run/显式清理 CLI 入口；下一步在完成真实库验证后评估 Redis 共享限流适配器。
