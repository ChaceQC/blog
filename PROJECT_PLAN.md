...
- 匿名访客识别采用企业级分层指纹方案：前端生成带版本号的 `VisitorFingerprint`，包含本地匿名设备 ID、浏览器/设备稳定信号哈希、Canvas/WebGL/Audio 字段摘要、语言、时区、屏幕特征和平台信息；前端只上送本地 SHA-256 后的摘要，不上送原始高维指纹细节。后端再结合可信代理解析出的 IP、UA、Accept-Language 等服务端风险信号，用 `BLOG_SECRET_KEY` 做 HMAC 派生并只保存派生结果。无登录场景下不能承诺绝对刷量不可破，但要提高无痕窗口重复点赞、短时重复访问和脚本重放的成本。
