...
- 混淆后的业务 chunk 仍保持多分片策略；冷缓存大量 hash JS 请求带来的压力通过 Nginx/API/WSS 限流余量、`/assets/` 长缓存和 WSS 请求顺序优化解决，不通过合并回少量大 JS 来降低防逆向分片强度。
...
- 日志中禁止输出密码、Token、Cookie、数据库连接串、对象存储签名 URL、公开访问 query/token 参数、内容标题/slug、外部 URL、文件名、MIME 和完整设置值；生产 Nginx 入口日志只能记录方法、`$uri`、协议、状态码、响应字节数、UA 和代理链，不记录 query 或 referrer。
...
- 文章修订历史，支持回滚。
- 公开文章支持匿名访问统计和点赞统计：文章列表只展示浏览数与点赞数；文章详情在右下角提供悬浮点赞按钮，并展示浏览数。
- 匿名访客识别采用企业级分层指纹方案：前端生成带版本号的 `VisitorFingerprint`，包含本地匿名设备 ID、浏览器/设备稳定信号哈希、Canvas/WebGL/Audio 字段摘要、语言、时区、屏幕特征和平台信息；`composite_hash` 与后端当前点赞身份派生逻辑保持兼容，本地匿名 ID 使用 localStorage 与同站 Cookie 双写互相恢复，降低本地存储异常导致身份漂移的概率。前端只上送本地 SHA-256 后的摘要，不上送原始高维指纹细节。后端再结合可信代理解析出的 IP、UA、Accept-Language 等服务端风险信号，用 `BLOG_SECRET_KEY` 做 HMAC 派生并只保存派生结果。无登录场景下不能承诺绝对刷量不可破，但要提高无痕窗口重复点赞、短时重复访问和脚本重放的成本。
...
- 文章详情支持匿名评论：公开访客无需登录即可提交评论，评论采用常见二级展示，可回复任意已发布评论但不继续增加缩进层级；回复二级评论时仍归入同一个顶层楼层，并用 `reply_to_id` 和回复目标昵称快照展示“回复 @某某”。顶部表单只用于顶层评论，回复表单显示在被回复评论附近。公开评论按顶层楼层分页，每页返回这些楼层及其下可公开显示的回复；公开页默认展示 5 条顶层评论、每个楼层 3 条回复，剩余评论通过“显示更多”展开；过长正文默认折叠，点击“展开”后显示全文。评论默认进入审核；提交成功后服务端只返回一次单条评论删除凭证，前端保存为 localStorage receipt。刷新页面时，前端通过 `comments/owned` 加密接口分批提交本地 receipt，服务端校验删除 token 后返回属于当前浏览器的待审核评论，因此提交者能看到自己的评论处于“审核中”。
...
| GET | `/api/public/posts/{slug}` | 文章详情，返回浏览数与点赞数，不直接修改统计 |
| POST | `/api/public/posts/{slug}/view` | 按匿名访客指纹短时去重记录浏览，只接受加密请求体 |
| POST | `/api/public/posts/{slug}/like` | 设置当前匿名访客的点赞状态，只接受目标状态，不接受计数增减 |
| GET | `/api/public/posts/{slug}/comments` | 公开评论列表，按顶层楼层分页，返回已发布评论和必要删除占位，响应加密 |
...
| GET | `/api/admin/comments` | 评论审核列表，按状态分页 |
| PATCH | `/api/admin/comments/{id}/review` | 通过、拒绝、标记垃圾或审核删除评论，需要 `comment:review` 和 CSRF |
| DELETE | `/api/admin/comments/{id}` | 管理员删除评论，需要 `comment:review` 和 CSRF |
...
| comment_count | BIGINT UNSIGNED | 已发布评论数 |
...
| visitor_hash | CHAR(64) | 匿名访客 HMAC 哈希，用于同一设备点赞/取消点赞 |
| fingerprint_hash | CHAR(64) | 客户端设备指纹哈希的服务端派生值 |
| risk_hash | CHAR(64) | 服务端风险指纹派生值，用于无痕和脚本刷量风控 |
| active | TINYINT | 当前是否点赞 |
| created_at | DATETIME(6) | 创建时间 |
| updated_at | DATETIME(6) | 更新时间 |

索引建议：

- 唯一索引：`uk_post_likes_post_visitor(post_id, visitor_hash)`，确保同一匿名设备对同一文章只有一条点赞状态记录。
- 普通索引：`idx_post_likes_post_active(post_id, active)`，用于统计与后台排查。

匿名互动安全边界：

- 所有公开互动接口继续使用 public scope 的 `content-v1` 加密会话，请求与响应均消费一次性 salt lease，重放同一 HTTP 请求会被拒绝。
- 点赞接口请求体使用严格 schema，禁止额外字段和非布尔目标状态；任何 `+n`、`-n`、`count`、`delta` 等字段都会被 Pydantic 拒绝或忽略为非法请求。
- 计数只由服务端根据数据库中 `post_likes.active` 的状态转换更新，更新语句使用固定 `+1` 或带下限保护的 `-1`，不读取客户端传入的数字。
- 浏览统计只通过后端 Redis/内存去重令牌决定是否增加 `posts.view_count`，前端不能提交浏览增量；去重 key 使用文章 ID 和服务端风险指纹派生值，降低清理本地存储或无痕窗口轮换匿名 ID 带来的重复计数。
- 匿名设备指纹只能作为无登录场景的风控与体验基础；用户清理浏览器数据、更换浏览器、切换设备、代理 IP 或篡改客户端环境时仍可能绕过，因此还需要配合 IP/风险指纹限流和日志监控。
- 文章软删除时，服务层必须清理该文章的匿名点赞记录并重置展示计数；物理删除时 `post_likes.post_id` 使用 `ON DELETE CASCADE` 兜底，确保不会留下孤儿互动数据。
...
| delete_token_hash | CHAR(64) | 单条删除凭证 HMAC，删除后置空 |
| body_text | LONGTEXT | 纯文本评论正文，删除后清空 |
| body_hash | CHAR(64) | 正文 HMAC，用于短窗口重复检测 |
| reply_count | BIGINT UNSIGNED | 直接回复数量 |
| reviewed_at | DATETIME(6) | 审核时间 |
| reviewed_by | BIGINT UNSIGNED | 审核管理员 |
| deleted_at | DATETIME(6) | 删除时间 |
| deleted_reason | VARCHAR(255) | 删除或拒绝原因分类，不放正文 |
| created_at | DATETIME(6) | 创建时间 |
| updated_at | DATETIME(6) | 更新时间 |

索引建议：

- 普通索引：`idx_post_comments_post_status_created(post_id, status, created_at, id)`，用于公开列表。
- 普通索引：`idx_post_comments_post_parent_status_created(post_id, parent_id, status, created_at, id)`，用于回复查询。
- 普通索引：`idx_post_comments_author_created(author_key_hash, created_at)`，用于 owned 找回和作者限流。
- 普通索引：`idx_post_comments_risk_created(risk_hash, created_at)`，用于风险桶限流。
- 普通索引：`idx_post_comments_post_body_created(post_id, body_hash, created_at)`，用于同文章重复正文检测。
...
- 公开列表只返回 `published` 评论，以及有回复时必须保留结构的删除占位；`limit` / `offset` 按顶层楼层分页，响应同时返回已发布评论总数 `total` 和顶层楼层总数 `thread_total`；`posts.comment_count` 只统计已发布评论。
...
- Nginx 按路径把公开/后台文件下载标记为 `bulk`，把文章图片渲染、缩略图、后台预览和头像缓存标记为 `media`，其余页面、API、RSS/sitemap/robots、登录和 WSS 标记为 `high`；该分类用于请求限流、并发兜底和后续宿主机 TC/HTB/CAKE QoS 打标，不等同于已经实现动态出口带宽抢占。
...
- 登录、加密协商、刷新令牌、上传、友链申请、搜索等接口做限流。当前 M1 已对后台登录、加密协商入口和公开友链申请入口接入可配置限流，并在命中时写入 `security_events`；加密 session 协商默认提高到 `120/分钟`，公开/后台活跃 session 默认上限分别为 `60` 和 `30`，避免冷缓存刷新或多公开查询并发时只完成 session 协商就被挡住。本地默认使用单进程内存限流，生产必须通过 `BLOG_RATE_LIMIT_BACKEND=redis` 和 `BLOG_REDIS_URL` 使用 Redis 共享限流、去重和 salt lease 状态；生产配置缺失 Redis 时应用启动应失败。应用只在直接连接来源属于 `BLOG_TRUSTED_PROXY_HOSTS` 配置的 IP 或 CIDR 时信任 `X-Forwarded-For` / `X-Real-IP`，避免后端直连时伪造客户端 IP。
...
- 应用层加密派生使用的固定 HKDF salt 必须迁移为 WSS 下发的一次性动态 salt lease，覆盖 AES-GCM JSON 信封和 Login Capsule v2。`esid` Cookie 本身使用 session/scope 绑定的稳定小 token，每个 HTTP 请求再额外消费一次性 `purpose=esid` lease 作为新鲜度因子。核心 crypto path 的 HKDF `info`、AES-GCM AAD、WSS 包裹上下文、`esid` 派生和 Login Capsule 派生不再使用可读协议字符串，统一由 `context_seed`、数字 opcode、scope/profile/purpose 数字 id 与 session/challenge/lease hash 组成二进制上下文。
- 核心 crypto path 不应在前端 chunk 中保留可读协议上下文字符串。`HKDF`、`AES-GCM`、`HMAC`、`SHA-256`、`AES-CTR` 等 WebCrypto primitive 名称运行时必然可见，但 JSON 信封、WSS salt 包裹、`esid` 和 Login Capsule 的 HKDF `info` / AES-GCM `additionalData` 必须改为由 session 级随机 `context_seed`、数字 opcode、scope/profile/purpose 数字 id、session/challenge/lease 哈希等组成的二进制上下文。该改动不兼容旧加密 session；上线后旧短期 session 直接失效并由浏览器重新协商。
- WSS salt 协议使用 `/api/{scope}/encryption/salts`：前端完成 `/api/{scope}/encryption/sessions` 的 P-256 ECDH 协商后，使用 `session_id` 建立 WSS；服务端按用途下发加密后的 salt lease 包。WSS 包本身用 ECDH shared secret 派生的临时包裹密钥加密，包裹密钥的 HKDF salt 来自每帧随机 `wrap_salt`，避免再引入前后端固定 salt。明文 lease 至少包含 `lease_id`、`purpose`、`scope`、`profile`、`salt`、`expires_at` 和 `sequence`；前端只缓存未过期、未使用的 lease。前端创建 HTTP 加密请求头时必须先启动 WSS 并批量获取当前请求的 `purpose=esid` 与 `purpose=response` lease，再生成/写入 `esid` Cookie，避免 `esid` 生成失败导致 WSS 根本不启动。WSS 长连接必须使用同一加密包裹格式发送应用层 `ping` / `pong` 心跳，浏览器端约每 25 秒发送一次加密 `ping`，连续 2 次未收到加密 `pong` 时主动关闭并重连；后端超过约 90 秒没有收到有效加密帧时关闭连接，并对每条 WSS 消息按客户端 IP 与 `session_id` 双维度执行消息级限流。
- WSS 重连必须重新建立 WebSocket 连接并继续使用当前未过期的加密会话密钥；重连采用指数退避并加入随机抖动，最大退避约 30 秒。断线或重连时不得降级回固定 salt，也不得复用尚未绑定到 HTTP 请求的本地 salt 缓存；已经绑定到在途 HTTP 响应的 response salt 可以保留到该响应完成或解密失败后清理，避免网络抖动直接破坏已发出的请求。若服务端以 `1008` 策略关闭 salt WSS，前端必须按当前加密 session 最近发送的 salt WSS 消息窗口估算剩余限流时间并进入动态冷却；冷却期内排队和新增的 salt 请求应快速失败，不得逐个重新建立 WSS 连接造成限流雪崩。
...
- 评论正文和昵称按纯文本保存与展示，公开评论区和后台审核页都依赖 React 文本转义；禁止用 `dangerouslySetInnerHTML`、`innerHTML`、`insertAdjacentHTML` 或字符串模板渲染评论字段。评论提交默认进入审核队列，并启用 IP、风险桶、匿名作者、重复正文和待审核总量限制；评论删除 token 只允许在加密请求体中传输。
...
- 记录结构化访问日志和应用日志；后台短时链接生成、后台文件下载、公开友链申请、公开站点跳转首访、RSS/sitemap/robots 正常响应和错误访问写入 `access_logs`。成功 `GET/HEAD` 默认通过 `BLOG_ACCESS_LOG_DEDUPE_SECONDS` 按 IP、方法和 path 做短时去重，公开站点跳转按 IP 和 item id 去重点击计数与访问日志，不把 query、临时 token、签名参数、slug、文件名、MIME 或列表数量写入访问日志明细；生产 Nginx access log 使用 `$uri` 替代完整 `$request`，并不记录 referrer，避免入口日志持久化签名 token；后台审计日志只保留动作、实体 id、操作者、状态类摘要和 changed_fields，不保存标题、URL、文件名、正文或完整设置值；后端容器运行访问日志读取同一份 `BLOG_TRUSTED_PROXY_HOSTS`，按可信代理头显示真实 IP 并输出时间戳，时间戳遵循容器 `TZ`/`tzdata` 且镜像默认启用 UTF-8；历史访问、审计、登录和安全事件日志由 `cleanup-logs` 维护任务按保留天数清理。
- 遥测上报使用 Project API Key，不复用后台登录 token，不由客户端传 `project_id`。第一阶段只允许服务端、后台维护任务和部署脚本持有遥测 API Key；浏览器前端如需 Web Vitals 或 JS 错误上报，必须通过后端受控转发。后端通过 `BLOG_TELEMETRY_ENABLED`、`BLOG_TELEMETRY_ENDPOINT` 和 `BLOG_TELEMETRY_API_KEY` 控制是否上传，默认关闭。遥测数据契约见 `docs/telemetry-reporting-design.md`，覆盖 HTTP 耗时/错误、限流、加密会话、salt WSS、公开文章互动、文件上传/访问、后台审计、友链申请、站点跳转、维护任务和部署完成事件；所有 tags 必须使用路由模板和低基数字段，不上报正文、slug、完整 URL/query、签名 token、Cookie、加密材料、原始 IP、完整 UA、文件名、MIME、外部 URL 或完整设置值。
...
| 匿名评论滥用 | 垃圾评论、待审核队列膨胀、重复提交 | 默认待审核，多维限流，重复正文检测，全站待审核上限 |
| 评论 XSS 或后台注入 | 恶意正文在公开页或审核页执行脚本 | 评论只按纯文本渲染，不支持 HTML/Markdown/外部资源，日志与审计不保存正文 |
| 评论删除凭证泄漏 | token 进入 URL、日志、遥测或数据库明文 | token 只在加密请求体传输，服务端只存 HMAC，删除后置空，日志和遥测白名单脱敏 |
| 匿名删除权丢失 | 用户清理浏览器数据后无法自助删除 | 明确产品边界，receipt 只保存在本地，丢失后由管理员处理 |
