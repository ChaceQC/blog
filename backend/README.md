...
遥测上报数据契约位于根目录 `docs/telemetry-reporting-design.md`。遥测摄入使用 Project API Key，不使用后台登录 token，也不由本项目传 `project_id`；第一阶段只允许后端、维护任务和部署脚本持有 API Key。推荐优先在 HTTP middleware、`record_admin_audit()`、`record_public_access()`、`enforce_rate_limit()`、加密会话/salt WSS、文件服务、公开文章互动服务和 CLI 维护任务处挂载 metrics/events/logs/traces。上报前必须沿用现有脱敏边界：不发送正文、slug、完整 URL/query、签名 token、Cookie、加密材料、文件名、MIME、外部 URL 或完整设置值。
