...
遥测上报数据契约位于根目录 `docs/telemetry-reporting-design.md`。遥测摄入使用 Project API Key，不使用后台登录 token，也不由本项目传 `project_id`；第一阶段只允许后端、维护任务和部署脚本持有 API Key。`BLOG_TELEMETRY_ENABLED=false` 时不会发送遥测；启用时还必须在后端 `.env` 或生产 `backend.env` 中提供 `BLOG_TELEMETRY_ENDPOINT` 和 `BLOG_TELEMETRY_API_KEY`。当前已接入 HTTP middleware、`record_admin_audit()`、`enforce_rate_limit()`、加密会话/salt WSS、文件上传/访问、公开文章互动、友链/站点跳转、CLI 维护任务和部署完成事件。上报前必须沿用现有脱敏边界：不发送正文、slug、完整 URL/query、签名 token、Cookie、加密材料、文件名、MIME、外部 URL 或完整设置值。
...
uv run python -m app.cli record-deployment-finished --status ok --duration-seconds 30 --git-sha abc123
```
