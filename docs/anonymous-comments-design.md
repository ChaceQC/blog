...
- 不做无限嵌套楼中楼。第一版采用常见二级评论展示：可以回复任意已发布评论，但所有回复都归并到同一个顶层楼层下，不继续增加第三层缩进。
...
- 未填写时显示 `匿名读者 #A1B2C3`。
- 同一篇文章中，不同匿名作者不能使用完全相同的自定义昵称；后端检测到冲突时自动追加 4 到 8 位字母数字后缀，例如 `访客#A1b2`。同一个匿名作者重复使用同一基础昵称不视为冲突，并复用自己此前的展示名。
...
- `parent_id` 必须存在、属于同一篇文章、且是已发布评论；它可以指向顶层评论或二级回复。后端保存时把 `parent_id` 归并为顶层楼层 id，并额外保存实际回复目标 `reply_to_id` 与 `reply_to_display_name` 快照。
...
| `parent_id` | BIGINT nullable FK | 顶层楼层 id，空值表示顶层评论 |
| `reply_to_id` | BIGINT nullable FK | 实际回复目标评论 id，目标删除后可置空 |
| `reply_to_display_name` | VARCHAR(64) nullable | 回复目标昵称快照，用于目标删除后继续展示“回复 @某某” |
...
| `display_name_base` | VARCHAR(32) nullable | 用户提交的基础昵称，用于同一作者复用展示名 |
...
- `display_name` 和 `display_name_base` 可置空，公开只返回删除占位。
- 如果没有子回复，公开列表可直接不返回该评论。
- 如果有子回复，公开列表返回 tombstone：`评论已删除`。顶层评论删除但仍有回复时保留顶层占位；二级回复作为 `reply_to_id` 被后续回复引用时，即使自身被删除也保留占位，后续回复继续使用创建时保存的 `reply_to_display_name` 快照。
...
- 对任意已发布评论都显示“回复”按钮；回复二级评论时不再产生第三层缩进，而是在同一顶层楼层下显示“回复 @某某”。
- 顶部评论表单只用于顶层评论；点击某条评论的“回复”后，在该评论附近显示内联回复表单。
- 顶层评论默认展示 5 条，剩余通过“显示更多评论”继续展开；每个顶层楼层下的回复默认展示 3 条，剩余通过小号“显示更多回复”继续展开。本地持有 receipt 的评论即使排在折叠范围外，也应被强制展示，保证提交者可见自己的审核中评论。
- 评论表单包含昵称和正文。昵称本地记忆。
- 过长评论正文默认折叠，点击“展开”后显示全文；折叠与展开都必须保持纯文本渲染。
